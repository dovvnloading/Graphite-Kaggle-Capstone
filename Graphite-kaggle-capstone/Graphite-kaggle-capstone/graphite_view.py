...
from graphite_canvas_items import Frame, Note, NavigationPin
from graphite_chart_item import ChartItem
from graphite_widgets import CustomScrollBar, ControlsPanel, SearchOverlay, PinOverlay
...
class ChatView(QGraphicsView):
    ...
    def __init__(self, window):
        ...
        # Initialize unified Controls Panel
        self.controls_panel = ControlsPanel(self)
        self.controls_panel.setVisible(False)
        
        # Connect Control Signals
        self.controls_panel.dragSpeedChanged.connect(self._update_drag_factor)
        
        self.controls_panel.gridOpacityChanged.connect(self._on_grid_opacity_changed)
        self.controls_panel.gridSizeChanged.connect(self._on_grid_size_changed)
        self.controls_panel.gridStyleChanged.connect(self._on_grid_style_changed)
        self.controls_panel.gridColorChanged.connect(self._on_grid_color_changed)
        self.controls_panel.snapToGridChanged.connect(self._on_snap_toggled)
        self.controls_panel.orthogonalConnectionsChanged.connect(self._on_ortho_toggled)
        self.controls_panel.smartGuidesChanged.connect(self._on_guides_toggled)
        
        self.controls_panel.fontFamilyChanged.connect(self.scene().setFontFamily)
        self.controls_panel.fontSizeChanged.connect(self.scene().setFontSize)
        self.controls_panel.fontColorChanged.connect(self.scene().setFontColor)
        
        self.controls_panel.closed.connect(lambda: self.controls_panel.setVisible(False))

        # Grid State (now managed here since GridControl is gone)
        self.grid_size = 10
        ...
        self.pin_overlay = PinOverlay(self)
        self.pin_overlay.setVisible(False)
        # Ensure closing the overlay via its own UI button hides it here
        self.pin_overlay.closed.connect(lambda: self.pin_overlay.setVisible(False))
    ...
    def _update_drag_factor(self, factor):
        self._drag_factor = factor
    ...
    def dropEvent(self, event):
        ...
        if event.mimeData().hasUrls():
            ...
            if urls:
                ...
                if hasattr(self.window, 'stage_dropped_file'):
                    self.window.stage_dropped_file(file_path)
        event.acceptProposedAction()
    ...
    def _update_overlay_positions(self):
        ...
        if self.controls_panel.isVisible():
            self.controls_panel.move(viewport_width - self.controls_panel.width() - padding, current_y_right)
            current_y_right += self.controls_panel.height() + padding
        ...
        if self.pin_overlay.isVisible():
            self.pin_overlay.move(padding, current_y_left)            
    ...
    def toggle_pins(self, visible):
        """Shows or hides the navigation pins overlay."""
        self.pin_overlay.setVisible(visible)
        self._update_overlay_positions()
    ...
    def mousePressEvent(self, event):
        ...
        if event.modifiers() & Qt.KeyboardModifier.ShiftModifier:
            ...
            # Use Application Override Cursor for robust "stuck" prevention
    ...
    def mouseMoveEvent(self, event):
        ...
        Args:
            event (QMouseEvent): The mouse event.
        """
        # Failsafe: If panning is True but Middle Button is NOT pressed, stop panning.
        if self._panning and not (QGuiApplication.mouseButtons() & Qt.MouseButton.MiddleButton):
            self._panning = False
            self._last_mouse_pos = None
            QApplication.restoreOverrideCursor() # Restore cursor safely
    ...
    def drawBackground(self, painter, rect):
        ...
        Optimized to use drawPoints for dots style.
        ...
        if zoom > LOD_MAJOR:
            ...
            # Optimization: Use drawLines logic if possible, but loop is standard for infinite grid
        ...
        if zoom > LOD_MINOR:
            ...
            if style == 'Lines':
                ...
                # OPTIMIZED: Collect points and draw once using drawPoints
                # This significantly reduces overhead compared to calling drawRect in a nested loop
                pen = QPen(minor_color, 2.0 / zoom) # Slightly thicker for visibility
                pen.setCapStyle(Qt.PenCapStyle.RoundCap)
                painter.setPen(pen)
                
                points = []
                # Pre-calculate range to avoid overhead in inner loop if possible, 
                # but standard range is efficient enough for typical viewports
                for x in range(minor_left, right, grid_size):
                    for y in range(minor_top, bottom, grid_size):
